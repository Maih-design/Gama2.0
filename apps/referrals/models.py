...
class Referral(models.Model):
    ...
    referral_center = models.ForeignKey(ReferralCenter, on_delete=models.PROTECT, related_name='referrals', verbose_name=_("المركز الموجه له الإحالة"))
    ...
    def clean(self):
        # 1. Verify procedure requires referral validation rule
        try:
            recommendation = self.committee_case.recommendation
            if not recommendation.procedure.requires_referral:
                raise ValidationError(_("لا يمكن إنشاء خطاب تحويل لهذه الحالة لأن الإجراء الطبي الموصى به لا يتطلب تحويلاً خارجياً."))
        except AttributeError:
            raise ValidationError(_("يجب إصدار توصية طبية معتمدة من اللجنة للحالة أولاً قبل التمكّن من إنشاء خطاب تحويل لها."))

        # 2. Only one active referral per case validation rule
        active_referrals = Referral.objects.filter(
            committee_case=self.committee_case
        ...
        if active_referrals.exists():
            raise ValidationError(_("توجد بالفعل إحالة أو خطاب تحويل ساري ومسجل لهذه الحالة الطبية."))
